...
class Trainer:
    def __init__(self, args, cfg, device):
        ...
        self.distill_ns = (
            True if self.args.distill and self.cfg.model.type in ["YOLOv6n", "YOLOv6s"] else False
        )
    ...
    def after_epoch(self):
        ...
        if self.main_process:
            ...
            # log for tensorboard
            write_tblog(
                self.tblogger, self.epoch, self.evaluate_results, lrs_of_this_epoch, self.mean_loss
            )
    ...
    def update_optimizer(self):
        ...
        if curr_step <= self.warmup_stepnum:
            self.accumulate = max(
                1, np.interp(curr_step, [0, self.warmup_stepnum], [1, 64 / self.batch_size]).round()
            )
