...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task):
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            for si, pred in enumerate(eval_outputs):
                ...
                if nl:
                    ...
                    self.scale_coords(
                        imgs[si].shape[1:], tbox, shapes[si][0], shapes[si][1]
                    )  # native-space labels
        ...
        if self.do_pr_metric:
            ...
            if len(stats) and stats[0].any():
                ...
                nt = np.bincount(
                    stats[3].astype(np.int64), minlength=model.nc
                ...
                LOGGER.info(
                    pf % ("all", seen, nt.sum(), mp, mr, f1.mean(0)[AP50_F1_max_idx], map50, map)
                )
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        ...
        if task != "speed" and len(pred_results):
            ...
            if self.is_coco:
                imgIds = [
                    int(os.path.basename(x).split(".")[0]) for x in dataloader.dataset.img_paths
                ]
            ...
            if self.verbose:
                ...
                mp = np.array(
                    [
                        np.mean(coco_p_iou50[ii][coco_p_iou50[ii] > -1])
                ...
                LOGGER.info(
                    ...
                    % (
                        "all",
                        val_dataset_img_count,
                        val_dataset_anns_count,
                        mp[i],
                        mr[i],
                        mf1[i],
                        map50,
    ...
    def eval_trt(self, engine, stride=32):
        ...
        for imgs, targets, paths, shapes in pbar:
            ...
            pred_results.extend(
                convert_to_coco_format_trt(nums, boxes, scores, classes, paths, shapes, self.ids)
            )
