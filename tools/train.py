...
def get_args_parser(add_help=True):
    ...
    parser.add_argument(
        "--conf-file", default="./configs/yolov6n.py", type=str, help="experiments description file"
    )
    parser.add_argument("--img-size", default=640, type=int, help="train, val image size (pixels)")
    parser.add_argument(
        "--rect", action="store_true", help="whether to use rectangular training, default is False"
    )
    ...
    parser.add_argument(
        "--dist_url", default="env://", type=str, help="url used to set up distributed training"
    )
    ...
    parser.add_argument(
        "--fuse_ab", action="store_true", help="fuse ab branch in training process or not"
    )
    parser.add_argument(
        "--bs_per_gpu",
        default=32,
